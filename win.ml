...
let views () =
	let rec show_texts prevs = function
		...
		| txt :: rest ->
			...
			let is_mapped = is_mapped (txt:>View.t) in
			let is_focused = is_focused (txt:>View.t) in
...
(* Commands dedicated to window management *)

let way_of_key dir =
	if dir = Term.Key.up    then Up else
	if dir = Term.Key.down  then Down else
	if dir = Term.Key.left  then Left else
	if dir = Term.Key.right then Right else
	invalid_arg "not a direction"

let install_win_commands =
	let c2i = Cmd.c2i
		...
		(try
			...
			let rec aux take_next = function
				...
				| (v, _, _, _, _) as vdescr :: rest ->
					if take_next then vdescr else aux (is_focused v) rest in
			match aux false views with (v, _, _, _, _) -> set_view v
	...
	| [ b ; p ] when b = c2i 'b' && p = c2i 'p' ->
		(try
			...
			let rec aux prev = function
				...
				| (v, _, _, _, _) as vdescr :: rest ->
					if is_focused v then prev else aux vdescr rest in
			match aux (List.hd views) (List.tl views) with (v, _, _, _, _) -> set_view v
		with Not_found -> Cmd.error "Not a single views?")
